...
term1 = sum(-Y .* log(a3));
term2 = sum((1 - Y) .* log(1 - a3));
J = sum(term1 - term2)/m;

reg = 0;
reg = reg + sum(sum(Theta1(:,2:input_layer_size+1) .^ 2));
reg = reg + sum(sum(Theta2(:,2:hidden_layer_size+1) .^ 2));
...
delta3 = a3 - Y;
activation = a2 .* (1 - a2);
delta2 = (Theta2' * delta3) .* (activation);
...
temp = (delta2 * X);
Theta1_grad = (Theta1_grad + temp(2:hidden_layer_size+1, :))/m;
